...
public class RunGrep {
	...
	public static void main(String[] args) {		
		...
		if (args.length == 5) {
			...
			sfilePattern=args[3];
			ssearchPattern=args[4];
		} else if (args.length == 6 && args[0].equals("-r")) {
			recursive = true;
			dirName = args[1];
			filePattern = args[2];
			searchPattern = args[3];
			sfilePattern=args[4];
			ssearchPattern=args[5];
		} else {
		...
		Map<File, List<String>> sresult = grep.grep(new File(dirName), sfilePattern, ssearchPattern, recursive);
		outputGrepResult(result);
		outputGrepResult(sresult);
	}
